...
const BookCard: React.FC<BookCardProps> = ({ book, variant = 'default' }) => {
  ...
  if (variant === 'compact') {
    return (
      <Link to={`/books/${book.id}`} className="group flex gap-4 p-4 card-book">
  ...
  return (
    <>
      <Link to={`/books/${book.id}`} className="group card-book overflow-hidden">
        {/* Cover Image */}
        <div className="relative aspect-[2/3] overflow-hidden bg-muted">
          ...
          {/* Quick action overlay — hidden for admin */}
          {!isAdmin && (
      ...
      {/* Modals — only render for non-admin */}
      {!isAdmin && (
        <>
          <SamplePdfViewer
            book={book}
